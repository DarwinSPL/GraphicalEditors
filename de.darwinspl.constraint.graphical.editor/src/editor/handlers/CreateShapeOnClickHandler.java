...
public class CreateShapeOnClickHandler extends AbstractHandler implements IOnDragHandler {
	...
	public void startDrag(MouseEvent event) {
	    ...
	    VBox n = ((VBox )getHost().getVisual().getParent().getParent());
	    AnchorPane p = (AnchorPane)n.getParent().getParent().getParent().getParent().getParent().getParent();
...
//				originInModel.getY());
		...
		copy.getTransform().setTransform(matrix[0], matrix[1], matrix[2], matrix[3], originInModel.getX()+p.getWidth(),
				originInModel.getY());
