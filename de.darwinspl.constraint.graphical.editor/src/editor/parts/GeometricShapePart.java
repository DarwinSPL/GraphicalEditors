...
public class GeometricShapePart extends AbstractGeometricElementPart<GeometryNode<IShape>>
	...
	public void setRedLayoutRectangleVisibility(boolean value) {
		layoutBoundsRect.setVisible(value);

	}

	public boolean getRedLayoutReclangleVisibility() {
		return layoutBoundsRect.isVisible();
	...
	@Override
	public void doAttachToAnchorageVisual(org.eclipse.gef.mvc.fx.parts.IVisualPart<? extends Node> anchorage,
			String role) {
		// nothing to do
	...
	protected void doAddChildVisual(IVisualPart<? extends Node> child, int index) {
		if (child instanceof AbstractGeometricElementPart) {
			Bounds parentBounds = getVisual().getLayoutBounds();
			Bounds parentBoundsInC = getRoot().getVisual().localToScene(getVisual().localToScene(parentBounds));

			// Bounds childBounds = getVisual().getLayoutBounds();
			// Bounds childBInContent =
			// getRoot().getVisual().localToScene(getVisual().localToScene(childBounds));
			//
			// Bounds parentBounds = p.getVisual().getLayoutBounds();
			// Bounds parentBInContent =
			// p.getRoot().getVisual().sceneToLocal(p.getVisual().localToScene(parentBounds));

			GeometricShapePart childPart = (GeometricShapePart) child;

			double y2 = (parentBoundsInC.getMinY() + 2.0);
			double x2 = ((parentBoundsInC.getMinX() + 8));
			childPart.getContent().setTransform(new AffineTransform(1, 0, 0, 1, x2, y2));
			child.refreshVisual();
	...
	protected GeometryNode<IShape> doCreateVisual() {
		...
		// start full drag to be able to catch 'DRAG_OVER' events
		...
		if (!debugging) {
			...
			layoutBoundsRect.setStrokeWidth(1.0);
	...
	protected void doRefreshVisual(GeometryNode<IShape> visual) {
		...
		if (content.getBoundsInParent() != visual.getBoundsInParent()) {
			content.setBoundsInParent(visual.getBoundsInParent());
		}

		// apply effect
		super.doRefreshVisual(visual);

		// doRefreshAllChildBlockParts();
	}

	@Override
	protected void doRemoveContentChild(Object contentChild) {
		// getContent().getChildren().remove(contentChild);
	}
	...
	protected List<IContentPart<? extends Node>> getRelocateChildControlBlocks(List<IContentPart<? extends Node>> linked,
			IContentPart<? extends Node> parent) {
		@SuppressWarnings("unchecked")
		// List<IContentPart<? extends Node>> anch =
		// PartUtils.filterParts(PartUtils.getAnchoreds(parent, "link"),
		...
		for (IContentPart<? extends Node> node : anch) {
			if (!(linked.contains(node))) {
				...
				if(node instanceof ControlBlockPart){
					((ControlBlockPart) node).doAttachToAnchorageVisual(parent, "relink");
				}
	...
	public void resizeIncludingParent(int widthDifference) {
		...
		throw new UnsupportedOperationException("Need to implement resizeParent(resizeParent) for " + this.getClass());
