...
public class OperatorFixedBlockPart extends AbstractNodePart<GeometryNode<IShape>> {

	@Override
	public OperatorFixedBlockModel getContent() {

		return (OperatorFixedBlockModel) super.getContent();
	}
	...
	Tooltip toolTipMissingChild = new Tooltip("Child Operator Block needs to be attached.");

	@Override
	protected void doRefreshVisual(GeometryNode<IShape> visual) {

		OperatorFixedBlockModel content = getContent();
	...
	public void doRefreshAllChildBlockParts() {
		...
		for (IContentPart<? extends Node> anch : getAnchoredsRecursive(list, this)) {
			...
			if (anch instanceof GeometricShapePart) {
				((GeometricShapePart) anch).doAttachToAnchorageVisual(this, "relink");
			}
	...
	public void doAttachToAnchorageVisual(IVisualPart<? extends Node> anchorage, String role) {
		...
		if (anchorage instanceof OperatorMovableBlockPart) {
			...
			if ((getContent().getOperatorFixedBlockType().equals(OperatorFixedBlockType.AND_OPERATOR2)) || (getContent()
					.getOperatorFixedBlockType().equals(OperatorFixedBlockType.VALIDATE_NOT_OPERATOR))) {

				getContent().getTransform().translate(b11.getMaxX() - b22.getMaxX(), b11.getMinY() - b22.getMinY());
		...
		if (anchorage instanceof ControlBlockPart) {
			...
			getContent().getTransform().translate(newx - b22.getMaxX(), b11.getMinY() - b22.getMinY());
